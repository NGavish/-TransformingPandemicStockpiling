function Figure2_Severe_Revised
...
nexttile
gammaI=1/2; gammaH=1/4; pHospitalization=0.06; 
...
set(gcf,"Position",[471 315 1056 500])
% Save (Optional)
printGraph('./graphs/Figure2_Severe_Revised');

end

function lg = plotPanelA
    % Data for Severe (Index 2 from original code logic)
...
function plotPanelB(pHospitalization, duration, clinicalAttackRate, gammaI, gammaH, N)
    ...
    text(t_peak+0.1, (1.5e5 + max_val)/2 - 1.5e5, ' (~2.6x Capacity)', 'FontSize', 9);

    % Formatting
    title('Dynamic Model (CDC Severe)', 'FontSize', 12);
...
function plotPanelC()
    ...
    title('1918 Historical Reconstruction - west coast', 'FontSize', 12);
    ylabel('Daily Bed Demand');
    xlabel('Weeks into wave');
    ylim([0 1.1e6]);
    set(gca, 'YTick', 0:5e5:2e6);
