function Figure1_UK_Scenario
% --- Setup and Data Initialization ---
close all;
figure('Color', 'w', 'Position', [100, 100, 1000, 500]);
t = tiledlayout(1,2, 'TileSpacing', 'compact', 'Padding', 'compact');

% UK Population estimate (approximate for the 2015-2020 timeframe)
N_UK = 69.5e6; 

% UK Scenario Targets and Calibrated Values (From Table 2)
beta = 0.31; 
p = 0.1000;              % Effective probability of hospitalization per infection
gammaI = 0.2; 
gammaH = 0.25; 
clinicalAttackRate = 0.50; % 50% clinical attack rate
hosp_rate = 0.04;          % 4.0% Hospitalization rate of symptomatic cases
duration_weeks = 15; 
duration_days = duration_weeks * 7;

% --- Panel A: Static Model (Lower Bound) ---
nexttile
lg = plotPanelA_UK(N_UK, clinicalAttackRate, hosp_rate, duration_days, gammaH);
text(0.05, 0.95, 'A', 'Units', 'normalized', 'FontSize', 14, 'FontWeight', 'bold');
box on;

% --- Panel B: Dynamic Model (SIHR) ---
nexttile
plotPanelB_UK(N_UK, beta, hosp_rate, gammaI, gammaH, clinicalAttackRate, duration_days, duration_weeks);
...
printGraph('./graphs/Figure_UK_Scenario')
end

function lg = plotPanelA_UK(N, clinicalAR, hospRate, duration_days, gammaH)
    % Calculate flat distribution (Static Model) for the UK
    ...
    % Antiviral mitigation bounds (Assuming 30% to 60% effectiveness, same logic as US)
    ...
    hold on;
    % Scaled Background Zones for the UK (proportional to ~31k bed capacity threshold)
    x_limits = [0 2 2 0 0]; 
    % p6 = fill(x_limits, [1e4 1e4 2e4 2e4 1e4], [1 1 0], 'FaceAlpha', 0.2, 'EdgeColor', 'none');
    % p7 = fill(x_limits, [2e4 2e4 3.1e4 3.1e4 2e4], [1 0.6 0.6], 'FaceAlpha', 0.3, 'EdgeColor', 'none');
    p_line = yline(14800, 'r--', 'LineWidth', 2);
    ...
    title('Static Model (UK Lower Bound)', 'FontSize', 12);
    
    % Scale Y-axis relative to UK limits
    ylim([0 1.5e5]); 
    % set(gca, 'YTick', 0:1e4:6e4);
    ytickformat('%.0fk');
    ...
    lg = legend([b1, b2, p_line], ...
        {'Unmitigated', 'With Antivirals', 'Collapse Threshold (~15k)'}, ...
        'FontSize', 10);
    hold off;
end

function plotPanelB_UK(N, beta, p, gammaI, gammaH, clinicalAttackRate, duration_days, duration_weeks)
    % Initial state
    I0 = 1e-3; 
    y0 = [1-I0; I0; 0; 0; 0];
    
    % Unmitigated Simulation (Note: we use the direct beta, no fzero needed)
    ...
    hold on;
    % Backgrounds (Scaled to UK ~31k Collapse Threshold)
    x_lims = [0 duration_weeks duration_weeks 0 0];
    % fill(x_lims, [1e4 1e4 2e4 2e4 1e4], [1 1 0], 'FaceAlpha', 0.2, 'EdgeColor', 'none');
    % fill(x_lims, [2e4 2e4 3.1e4 3.1e4 2e4], [1 0.6 0.6], 'FaceAlpha', 0.3, 'EdgeColor', 'none');
    yline(14800, 'r--', 'LineWidth', 2);
    text(5.2, 14800, 'Free bed capacity (~15k)', 'Color', 'r', 'FontSize', 9, 'VerticalAlignment', 'bottom');
    ...
    % Formatting
    title('Dynamic Model (UK Scenario)', 'FontSize', 12);
    ylabel('Daily Bed Demand');
    xlabel('Weeks into wave');
    
    ylim([0 1.5e5]);
    % set(gca, 'YTick', 0:1e:6e4);
    ytickformat('%.0fk');
    labels = arrayfun(@(v) sprintf('%.0fk', v/1e3), get(gca, 'YTick'), 'UniformOutput', false);
    set(gca, 'YTickLabel', labels);
